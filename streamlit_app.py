...
st.set_page_config(page_title="Conformal Predictive Distribution", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Conformal Predictive Distribution</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Full predictive distributions | Finite-sample validity guarantees | Beyond prediction intervals</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 CPD")
...
st.sidebar.markdown(f"**Quantiles:** {config.N_QUANTILES} | **Trees:** {config.N_ESTIMATORS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'cpd_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Predictive density: {etf['cpd_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Density"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Density", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Predictive Density (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Conformal Predictive Distribution** produces full predictive distributions.
        - Provides **finite-sample validity guarantees** for any specified quantile.
        - Extends conformal prediction from intervals to full distributions.
        - The score is the **predictive density** at the observed next-day return.
        - Higher density → the model assigns high probability to the actual outcome → more accurate prediction.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the predictive densities.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Conformal Predictive Distribution | Full predictive distributions with finite-sample guarantees")
