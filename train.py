...
from conformal_distributional import conformal_distributional_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = conformal_distributional_score(
    ...
    top_etfs = [{"ticker": t, "cpd_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/cpd_{timestamp}.json"
